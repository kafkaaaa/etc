...
// 기대자원, 현재 할당된자원의 수, 최대 자원의 수, 현재 가용자원의 수  
int need[101][101], alloc[101][101], max[101][101], available[101];
	...
	while (count <= N) {
		...
		for (i=1; i<=N; i++) {
			if (finish[i] == false) {			// 작업이 완료되지 않았으면 
				for (j=1; j<=M; j++) {
					if(work[j] < need[i][j]) {	// 현재 가용자원 < 기대자원 
						break;					// 할당할 수 없음. 
					}
				}
				if (j > M) {					// 현재 가용자원 >= 기대자원 
...
void main()
{
	...
	// need[] = 작업을 완료하기까지 필요한(남은) 자원의 수 =  max[] - alloc[] 
	for (i=1; i<=n; i++) {
		for (j=1; j<=m; j++) {
    ...
    */
	...
	for (i=1; i<=m; i++) {
		...
		if ( need[indx][i] < request[i] || request[i] > available[i] ) {
	...
	if (i > m) {
		...
		for (i=1; i<=m; i++) {
			...
			need[indx][i] -= request[i];	// 필요자원 = 필요자원 - 요청자원 
		}
